...
export function generateRulePrediction(match) {
  ...
  const { homeGoals, awayGoals } = predictScore(
    homePower,
    awayPower,
    homeWin,
    draw,
...
function normalizeAiNumber(value, fallback = 0) {
  const numberValue = Number(value);
  return Number.isFinite(numberValue) ? numberValue : fallback;
}

function normalizeAiPrediction(data, match) {
  if (!data) {
    return generateRulePrediction(match);
  }

  return {
    result: data.result || data.prediction || "TBD",
    score: data.score || data.predictedScore || "TBD",

    homeWin: normalizeAiNumber(
      data.homeWin ?? data.home_win ?? data.homeWinProbability,
      0
    ),

    draw: normalizeAiNumber(
      data.draw ?? data.drawProbability,
      0
    ),

    awayWin: normalizeAiNumber(
      data.awayWin ?? data.away_win ?? data.awayWinProbability,
      0
    ),

    confidence: normalizeAiNumber(data.confidence, 0),

    analysis:
      data.analysis ||
      "Prediction generated by FastAPI XGBoost AI service.",
...
export async function generatePrediction(match) {
  ...
  if (!aiUrl) {
    console.error("AI_SERVICE_URL missing in backend .env");
    return generateRulePrediction(match);
  }
  ...
  try {
    console.log("Calling AI:", `${aiUrl}/predict`);

    const { data } = await axios.post(`${aiUrl}/predict`, payload, {
      timeout: 10000,
      headers: {
    ...
    return normalizeAiPrediction(data, match);
  } catch (error) {
    console.error("AI service failed, using fallback");
    console.error("AI URL:", aiUrl);
